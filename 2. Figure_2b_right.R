...
####不同组织tissue中不同subcelltype2(endo 亚组)比例
Ratio <- sce_VC@meta.data %>%group_by(tissue,subcelltype2) %>%
  count() %>%
  group_by(tissue) %>%
  mutate(Freq = n/sum(n)*100)

pdf(paste0("./",sam.name,"/Ratio_subcelltype2.",max(dim.use),"PC.pdf"),width = 4,height = 8)
