...
# ???ù???·??
workdir <- "/Users/yimang/Desktop/20230314Endo/泛癌表达/Endo"; setwd(workdir)
...
# ???ø???Ȥ?Ļ?????(TTC35??EMC2??ͬ??)
#frg2 <- c("CDKN1A","HSPA5","TTC35","SLC7A11","NFE2L2","MT1G","HSPB1","GPX4","FANCD2","CISD1","FDFT1","SLC1A5","SAT1",
#         "TFRC","RPL8","NCOA4","LPCAT3","GLS2","DPP4","CS","CARS","ATP5G3","ALOX15","ACSL4","EMC2")
frg2<- c("ESM1","PGF","NID2","COL4A1","COL4A2",
         "VWA1","APLN","LAMA4","MCAM",
         "CA4","CD36","CD320","BTNL9","FABP5","PLVAP","TMEM88","SCLC14A1")
...
exprTab$gene <- factor(exprTab$gene,
                       levels = rev(c("ESM1","PGF","NID2","COL4A1","COL4A2",
                                      "VWA1","APLN","LAMA4","MCAM",
                                      "CA4","CD36","CD320","BTNL9","FABP5","PLVAP","TMEM88","SCLC14A1")))
...
ggarrange(p_top,
          ...
          heights = c(2,6),
          common.legend = F)
ggsave("Figure 1B differential expression of interested genes in pancancer_33.pdf", width = 7,height =3.5)
rm(expr); gc()
