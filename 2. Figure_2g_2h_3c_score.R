...
pdf(paste0("./",sam.name,"/Angiogenesis_score",".pdf"),width = 10,height = 7)
VlnPlot(sce_VC,features = 'Angiogenesis_score1',pt.size = 0
        )+geom_boxplot(width=0.2,col="black",fill="white")
dev.off()
write.table(sce_VC@meta.data,"sce_VC@meta.data_Angiogenesis",sep="\t",quote = F)


#sce 的Angiogenesis_score
Angiogenesis_score <- readxl::read_xlsx("Angiogenesis_score.xlsx")
...
pdf(paste0("./",sam.name,"/Angiogenesis_score",".pdf"),width = 10,height = 10)
VlnPlot(sce,#group.by="celltype",#按字母顺序排序
        features = 'Angiogenesis_score1',pt.size = 0
)+geom_boxplot(width=0.2,col="black",fill="white")
dev.off()
write.table(sce@meta.data,"sce@meta.data_Angiogenesis",sep="\t",quote = F)
...
dev.off()
write.table(sce_BVEC@meta.data,"sce_BVEC@meta.data_Angiogenesis",sep="\t",quote = F)
